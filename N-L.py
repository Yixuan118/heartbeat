...
TEST_ON_BED_FILE = r"D:./data/raw_signal_before_2025-11-07T145144_2025-11-07T150056.npy"
TEST_UNDER_BED_FILE = r"D:./data/raw_signal_after_2025-11-07T145144_2025-11-07T150056.npy"
...
MODEL_NAME = "Hammerstein (Generalized N-Block + FRF L-Block)"
CHOSEN_FRF_ESTIMATOR = 'Hv_geometric_mean'
VOLTERRA_MEMORY_DEPTH_TO_TRY = [3, 5, 8, 10]

# [!! MODIFIED !!]
# 将 'Orders' (次方) 替换为 'Basis Functions' (基函数)
# 您可以混合使用 'x', 'x^2', 'x^3', 'symlog(x)', 'exp(x)', 'tanh(x)' 等。
BASIS_FUNCTIONS_TO_TRY = [
    ['x'],  # 仅线性
    ['x', 'x^3'],  # 奇次多项式
    ['x', 'x^2', 'x^3'],  # 混合多项式
    ['x', 'symlog(x)'],  # 线性 + 对称对数 (您的建议)
    ['x', 'exp(x)'],  # 线性 + 指数 (您的建议)
    ['x', 'tanh(x)'],  # 线性 + 饱和 (Tanh)
    ['x', 'x^3', 'symlog(x)']  # 混合
]
...
# --- N-Block Functions (Generalized) ---

# [!! NEW HELPER FUNCTION !!]
def apply_basis_function(v, func_str):
    """
    将指定的基函数（由字符串定义）应用于输入向量 v。
    处理了 log(x) 和 exp(x) 的稳定性问题。
    """
    with np.errstate(over='ignore', invalid='ignore'):
        ...
        # --- 您建议的函数 ---
        elif func_str == 'symlog(x)':
            # 对称对数: sign(x) * log(|x| + 1)
            # 这可以稳定地处理 0 和负值
            return np.sign(v) * np.log1p(np.abs(v))
        elif func_str == 'exp(x)':
            # 指数
            # 标准化后的 v 应该在合理范围内, 但我们仍然裁剪以防万一
            return np.exp(np.clip(v, -10, 10))

            # --- 其他有用的函数 ---
        elif func_str == 'tanh(x)':
            # 双曲正切 (一个S形/饱和函数)
            return np.tanh(v)
        elif func_str == 'abs(x)':
            # 绝对值 (另一个偶次函数)
            ...
            # 未知函数
            print(f"Warning: Unknown basis function '{func_str}'. Returning zeros.")
            return np.zeros_like(v)


# [!! MODIFIED FUNCTION !!]
def train_volterra_diag_nblock(input_segments, target_segments, memory_depth, basis_functions,
                               regularization_alpha=0.1):
    """
    训练广义N-Block。
    'basis_functions' 是一个字符串列表, e.g., ['x', 'x^3', 'symlog(x)']
    """
    basis_functions = sorted(list(set(basis_functions)))  # 去重
    num_basis_terms = len(basis_functions)
    if num_basis_terms == 0:
        print("Error: No valid basis functions provided for N-Block.");
        return None, None, None

    print(
        f"\nTraining Generalized N-Block (M={memory_depth}, Basis={basis_functions}, Alpha={regularization_alpha})...")
    ...
    M = memory_depth
    num_coeffs = num_basis_terms * M
    total_valid_rows = sum(len(s) - M + 1 for s in valid_inputs)
    if total_valid_rows == 0:
        print("Error: No valid rows for Volterra matrix (check memory depth vs segment length).");
        return None, None, None
    ...
    for i in range(len(valid_inputs)):
        ...
        for n in range(num_samples_in_seg):
            v_delayed = v_seg[n + M - 1: n - 1 if n > 0 else None: -1]  # [v(n), v(n-1), ..., v(n-M+1)]
            current_col_offset = 0

            # [!! MODIFIED !!]
            # 迭代基函数字符串, 而不是 'orders'
            for func_str in basis_functions:
                # 应用选择的基函数 (e.g., 'symlog(x)')
                v_delayed_transformed = apply_basis_function(v_delayed, func_str)

                # 检查
                if not np.all(np.isfinite(v_delayed_transformed)):
                    v_delayed_transformed = np.clip(v_delayed_transformed, -1e20, 1e20)
                    v_delayed_transformed[~np.isfinite(v_delayed_transformed)] = 0

                Phi_N[current_row, current_col_offset:current_col_offset + M] = v_delayed_transformed
...
# [!! MODIFIED FUNCTION !!]
def apply_volterra_diag_nblock(input_segments, coeffs, memory_depth, basis_functions, v_mean, v_std):
    """
    应用广义N-Block。
    'basis_functions' 是一个字符串列表, e.g., ['x', 'x^3', 'symlog(x)']
    """
    processed_segments = []
    basis_functions = sorted(list(set(basis_functions)))  # 确保顺序一致
    M = memory_depth

    if coeffs is None or len(coeffs) != len(basis_functions) * M:
        print(
            f"Error: N-Block coeffs are None or mismatch. Expected {len(basis_functions) * M}, got {len(coeffs)}. Returning zeros.")
        return [np.zeros_like(s) if len(s) > 0 else np.array([]) for s in input_segments]

    if not np.isfinite(v_mean) or not np.isfinite(v_std) or v_std < 1e-9:
        print("Error: Invalid mean/std for N-Block application.");
        v_mean = 0.0;
        v_std = 1.0

    for v in input_segments:  # 'v' 在这里代表 *输入* (x)
        if len(v) < M:
            processed_segments.append(np.zeros_like(v));
            continue

        v_scaled = (v - v_mean) / v_std  # 标准化 *输入* (x)
        y_pred = np.zeros_like(v_scaled, dtype=float)  # 这是中间信号 'w'
        current_coeff_idx = 0
        ...
        for func_str in basis_functions:
            ...
            # 应用选择的基函数 (e.g., 'symlog(x)')
            v_transformed = apply_basis_function(v_scaled, func_str)

            # 检查
            if not np.all(np.isfinite(v_transformed)):
                v_transformed = np.clip(v_transformed, -1e20, 1e20);
                v_transformed[~np.isfinite(v_transformed)] = 0

            # y_pred 是 *中间信号* (w)
            y_pred += signal.lfilter(h_coeffs, [1.0], v_transformed)
            current_coeff_idx += M
...
if __name__ == "__main__":
    ...
    print("=" * 70)
    all_results = [];
    best_result = {'M': 0, 'BasisFuncs': [], 'Alpha': 0, 'AmpErr': 999.0, 'Corr': 0.0}  # [!! MODIFIED !!]
    ...
    # [!! MODIFIED !!]
    total_combinations = len(VOLTERRA_MEMORY_DEPTH_TO_TRY) * len(BASIS_FUNCTIONS_TO_TRY) * len(N_ALPHA_TO_TRY)
    current_combination = 0

    # [!! MODIFIED !!]
    #  三重循环, 流程改为 N -> L
    for M in VOLTERRA_MEMORY_DEPTH_TO_TRY:
        for basis_funcs in BASIS_FUNCTIONS_TO_TRY:  # [!! MODIFIED !!]
            for n_alpha in N_ALPHA_TO_TRY:
                current_combination += 1
                basis_str = str(basis_funcs)  # [!! MODIFIED !!]
                param_str = f"M={M}, Basis={basis_str}, A={n_alpha}"  # [!! MODIFIED !!]
                ...
                train_intermediate_w = apply_volterra_diag_nblock(
                    ...
                    basis_funcs, v_mean_current, v_std_current  # [!! MODIFIED !!]
                )
                ...
                all_results.append((M, basis_str, n_alpha, test_amp_err, test_corr))  # [!! MODIFIED !!]
                ...
                if test_amp_err < best_result['AmpErr']:
                    ...
                    best_result = {'M': M, 'BasisFuncs': basis_funcs, 'Alpha': n_alpha, 'AmpErr': test_amp_err,
                                   # [!! MODIFIED !!]
                                   'Corr': test_corr}
    ...
    # [!! MODIFIED !!]
    print(
        f"{'M (N-Block)':<12} | {'Basis Functions':<25} | {'N_Alpha':<10} | {'Test Amp Err (%)':<18} | {'Test Correlation':<18}")
    print("-" * 90)
    all_results.sort(key=lambda x: x[3])  # Sort by AmpErr
    for M, B, A, AmpErr, Corr in all_results: print(
        f"{M:<12} | {B:<25} | {A:<10.2f} | {AmpErr:<18.2f} | {Corr:<18.4f}")  # [!! MODIFIED !!]

    print("\n" + "=" * 90);
    print("Best Model Found");
    print("=" * 90)
    best_basis_str = str(best_result['BasisFuncs'])  # [!! MODIFIED !!]
    print(f"N-Block Model:    Generalized (Dynamic)")  # [!! MODIFIED !!]
    print(f"N-Block Memory M: {best_result['M']}")
    print(f"N-Block Basis:    {best_basis_str}")  # [!! MODIFIED !!]
    ...
    if best_test_predicted_scaled is None:
        ...
        MODEL_NAME_BEST = f"Best {MODEL_NAME} (M={best_result['M']}, Basis={best_basis_str}, A={best_result['Alpha']:.2f})"  # [!! MODIFIED !!]
